...
long int voltageSolarValue = 0;      // Variable for A2
...
void loop() {
  ...
  for (int i = 0; i < averageValue; i++)    // Repeat the cycle
  {
    ...
    voltageSolarValue += analogRead(A2);
    delay(2);                               // Pause 2 µs
  }
  ...
  voltageSolarValue = voltageSolarValue / averageValue;
...
void function_transmit_all() {
  ...
  json += "\"slr\":" + String(voltageSolarValue) + ",";
  json += "\"adc_voltage\":" + String(voltage, 2) + ",";
  json += "\"current\":" + String(current, 2) + ",";
  json += "\"ard_voltage\":" + String(input_voltage, 2) + ",";
  json += "\"cnt\":" + String(counter) + ",";
  json += "\"cmd\":" + String(device_mode);
  json += "";
